...
class Syntactic:
        ...
        def __init__(self,chunk_size,token_size,num_authors,num_authors_list,sliding_window,num_paper):
                ...
                self.db_name = "syn_eng_max_while_np%s_c%s_t%s_a%s_al%s_sw%s" %(num_paper,chunk_size,token_size,num_authors,num_authors_list,sliding_window)
        ...
        def create_db_table(self):
                ...
                cur.execute("CREATE TABLE writes_hidden (author_id VARCHAR(20), paper_id VARCHAR(20),author_num VARCHAR(20), CONSTRAINT writes_hidden_pkey PRIMARY KEY (author_id, paper_id))")
                cur.execute("CREATE TABLE section (paper_id VARCHAR(20), section_id SERIAL, raw_text TEXT, novel_id VARCHAR(20), author_id VARCHAR(20), CONSTRAINT section_pkey PRIMARY KEY (paper_id,section_id))")
                cur.execute("CREATE TABLE chunk (paper_id VARCHAR(20), start_sec VARCHAR(20), end_sec VARCHAR(20), chunk_id VARCHAR(20), CONSTRAINT chunk_pkey PRIMARY KEY (paper_id,chunk_id))")
                cur.execute("CREATE TABLE features (paper_id VARCHAR(20), chunk_id VARCHAR(20), feature_id SERIAL, value VARCHAR(50),CONSTRAINT feature_pkey PRIMARY KEY (paper_id,chunk_id,feature_id))")
        ...
        def save_section_features_to_db(self,list_authors,list_authors_id_200):
                ...
                for i in range(0,self.num_paper):
                        ...
                        for j in range(0,self.num_authors):
                                ...
                                cur.execute("INSERT INTO section VALUES(%s,%s,%s,%s,%s)",[i+1,num_section,raw_novel_text,novel_id,list_authors[i][j]])
                                num_section += 1
...
def parse_args():
    ...
    parser.add_argument('--sliding_window', type=int,
...
if __name__ == "__main__":
        args = parse_args()
        syn_dataset = Syntactic(chunk_size = args.chunk_size,token_size = args.token_size, num_authors = args.num_authors, num_authors_list = args.num_authors_list, sliding_window = args.sliding_window,num_paper = args.num_paper)
