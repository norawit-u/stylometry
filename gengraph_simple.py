...
class Gengraph:
    ...
    def generate_paper(self):
        ...
        for j in range(0,self.num_papers):
            ...
            for i in range(0,4):
                fragment_id +=1
                new_fragments[fragment_id] = author_list[int(i/2)] #frag_id = author_list[i]
    ...
    def get_similar_fragments(self,papers,paper_id,fragment_id):
        ...
        for i in range(1,len(x)):
            ...
            paper_id2 = math.ceil(fragment_id2/float(4))
            author_id = papers[paper_id]['fragments'][fragment_id]
            if paper_id2 > 2000:
                paper_id2 = 2000
            if author_id > 4000:
    ...
    def checking_accuracy_fragments(self,papers,frag_probs):
        ...
        for x in range(0,len(papers)):
            print(frag_probs[x+1],(x+1)*self.num_authors)
            sum_prob[x+1] = {k:0 for k in frag_probs[x+1][(x+1)*self.num_authors]}
            print(sum_prob)
            for y in frag_probs[x+1].keys():
                print(x,y,frag_probs[x+1][y].items(),sum_prob[x+1],int(y/2)+1)
                sum_prob[x+1] = {k:sum_prob[x+1][k]+v for k,v in frag_probs[x+1][y].items()}
            sum_prob[x+1]={k:sum_prob[x+1][k]/self.num_authors for k in frag_probs[x+1][(x+1)*self.num_authors]}
        for z in sum_prob:
            list_check.append(sorted(sum_prob[z].iteritems(),key=operator.itemgetter(1),reverse=True)[0:self.num_authors])
        count_all = 0
        count = 0
        count_least_1 = 0
