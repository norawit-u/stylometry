...
class Syntactic:
    ...
    def get_top_author_who_write_paper(self, shuffle=False):
        ...
        cur.execute("select distinct(paper_id) from raheem_author_paper where author_id in ( select a.author_id from (select author_id, count(*)  from raheem_author_paper group by author_id order by count(*) DESC limit 143) as a);")
