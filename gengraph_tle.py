...
class Gengraph:
    def __init__(self, num_authors, num_authors_list, papers, db_name, fname):
        ...
        self.fname = fname
        self.num_frag = self.get_num_fragment(20,5,40)
    ...
    def get_authors_list_all(self):
        con = psycopg2.connect("dbname ='%s' user='cpehk01' host=/tmp/" % (self.db_name.lower()))
        cur = con.cursor()
        cur.execute("SELECT author_id FROM writes_hidden group by author_id")
        get_list = cur.fetchall()
        list_return = []
        for i in range(0,len(get_list)):
            list_return.append(get_list[i][0])
    ...
    def generate_paper(self):
        ...
        for j in self.papers:
            ...
            num_frag = self.get_num_fragment(20,5,40)
            for i in range(0, num_frag):
                fragment_id = frag_id
                frag_id = frag_id + 1
                new_fragments[fragment_id] = author_list  # frag_id = author_list[i]
    ...
    def generate_frag_probs(self,papers):
        frag_probs = {}
        #authors = self.get_authors_list_all()
        #print(len(authors))
        for (paper_id,v) in papers.items():
            authors = self.get_authors_list(str(paper_id))
    ...
    def get_similar_fragments(self,papers,paper_id,fragment_id):
        ...
        for i in range(1, len(x)):
            ...
            paper_id2 = int(math.floor((fragment_id2) / float(self.num_frag)))+1 #get paper_id2 from fragment_id2
    ...
    def checking_accuracy_fragments(self, papers, frag_probs):
        ...
        for key, z in enumerate(sum_prob):
            list_check[self.papers[key]] = sorted(sum_prob[z].items(), key=operator.itemgetter(1), reverse=True)[0:self.num_authors]
        count_all = 0
        count = 0
        count_least_1 = 0
