...
#        data += (str(LSHMHDLongListTime) + "\n")
        start = time.time()
        sortedListLSH = stylometry.MHDPrunList(queryParaList, ListLSH_ALL, combinedhitQP, datasetm, paraIndex, R, hitparaIndex, doc_hit_above_T_dict, "lsh", MHDRatio)
        print(("MHDPrunList %s"%(time.time()-start)))
        LSH_MHD_Result, LSH_MHD_values = stylometry.geneMHDPrun(sortedListLSH, queryParaList, combinedhitQP, topknn, flagNum, datasetm, paraIndex, hitparaIndex, MHDRatio, "lsh")
